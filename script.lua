--[[
    ...
    FIX v11.3.2:
      - weatherMap ditambahkan (sebelumnya hilang → Shop error)
      - _G.SelectedStoneType / _G.TargetEnchantBasic diinit di awal
      - Ukuran UI: 420x560 → 580x460 (sama seperti WindUI)
      - Posisi center disesuaikan
      - Tween startup & close disesuaikan ukuran baru
      - Duplikasi definisi Notify dihapus
]]
...
-- =============================
--    FIX: Inisialisasi global sebelum dipakai
-- =============================
...
-- =============================
--    FIX: weatherMap (sebelumnya hilang → Shop error)
-- =============================
local weatherMap = {
    ["Windy (10k)"]       = "Wind",
    ["Cloudy (20k)"]      = "Cloudy",
...
local function ub_loop()
    while Config.UB.Active do
        local ok, err = pcall(function()
            ...
            if Config.antiOKOK then task.wait(math.random(15, 25) / 100)
        ...
        if not ok then warn("[MNA HUB] UB error: " .. tostring(err)); task.wait(1) end
    end
end
        ...
        while Config.AutoSellState do
            if not Events.sell then
                ...
                if not Events.sell then
                    ...
                    Config.AutoSellState = false; break
                end
            end
            if Config.AutoSellMethod == "Delay" then
                task.wait(math.max(Config.AutoSellValue, 5))
                if Config.AutoSellState then pcall(function() Events.sell:InvokeServer({}) end) end
    ...
    if enabled then
        walkOnWaterConn = RunService.Heartbeat:Connect(function()
            ...
            if hrp.Position.Y < waterY then
                hrp.CFrame = hrp.CFrame + Vector3.new(0, waterY - hrp.Position.Y, 0)
            end
        end)
...
local function setHideNameTag(enabled)
    ...
    pcall(function()
        ...
        if hum then
            ...
            hum.HealthDisplayDistanceType = enabled and Enum.HumanoidDisplayDistanceType.None or Enum.HumanoidDisplayDistanceType.Viewer
        end
    end)
    if enabled then NotifySuccess("Hide NameTag", "Nama disembunyikan!")
    else NotifyWarning("Hide NameTag", "Nama ditampilkan kembali.") end
end
...
local function setDisableObtained(enabled)
    ...
    if enabled then
        local xr = Events.ObtainedNotif or GetServerRemote("RE/ObtainedItemNotification")
        if xr then
            pcall(function()
                for _, conn in pairs(getconnections(xr.OnClientEvent)) do
                    if conn.Function then
                        conn:Disable()
                        table.insert(obtainedDisabledConns, conn)
                    end
                end
            end)
        end
        ...
        for _, conn in ipairs(obtainedDisabledConns) do
                ...
                if not eggFolder then
                    for _, obj in pairs(workspace:GetDescendants()) do
                        if obj:IsA("BasePart") and obj.Name:lower():find("egg") then
                            ...
                            if hrp then
                                ...
                                task.wait(0.3)
                                if Events.CollectEgg then pcall(function() Events.CollectEgg:FireServer(obj.Name) end) end
                                task.wait(0.2)
                            end
                        end
                    end
                    ...
                    for _, egg in pairs(eggFolder:GetChildren()) do
                        ...
                        if hrp and egg:IsA("BasePart") then
                            ...
                            task.wait(0.3)
                            if Events.CollectEgg then pcall(function() Events.CollectEgg:FireServer(egg.Name) end) end
                            task.wait(0.2)
                        end
                    end
                end
                                ...
                                if pos then
                                    ...
                                    for _, remote in pairs(net:GetChildren()) do
                                        ...
                                        or remote.Name:lower():find("interact") then
                                            pcall(function()
                                                if remote:IsA("RemoteEvent") then remote:FireServer(obj.Name) end
                                            end)
                                        end
                                    end
                                    task.wait(0.3)
...
local function createPingPanel()
    ...
    pingGui.Name = "MNA_PingPanel"; pingGui.ResetOnSpawn = false
    ...
    lblTitle.Size = UDim2.new(1,0,0.45,0); lblTitle.BackgroundTransparency = 1
    lblTitle.Text = "MNA HUB — STATS"; lblTitle.TextColor3 = Color3.fromRGB(200,50,50)
    ...
    lblStats.Size = UDim2.new(1,0,0.55,0); lblStats.Position = UDim2.new(0,0,0.45,0)
    lblStats.BackgroundTransparency = 1; lblStats.Text = "Ping: -- ms  |  FPS: --"
    lblStats.TextColor3 = Color3.fromRGB(240,240,240); lblStats.TextSize = 11
    lblStats.Font = Enum.Font.Gotham; lblStats.TextXAlignment = Enum.TextXAlignment.Center

    local fpsCount, fpsAccum = 0, 0
    local fpsConn2 = RunService.RenderStepped:Connect(function(dt)
...
local function applyAura(auraName)
    ...
    if not data then NotifyWarning("Aura", "Aura dihapus."); return end
    ...
    particle.Size         = NumberSequence.new({ NumberSequenceKeypoint.new(0,0.5), NumberSequenceKeypoint.new(0.5,1.2), NumberSequenceKeypoint.new(1,0) })
    particle.Transparency = NumberSequence.new({ NumberSequenceKeypoint.new(0,0),   NumberSequenceKeypoint.new(0.5,0.4), NumberSequenceKeypoint.new(1,1) })
    particle.LightEmission = 0.8; particle.LightInfluence = 0.2
    particle.Speed         = NumberRange.new(data.speed, data.speed * 2)
    particle.Rate          = 30;  particle.Lifetime = NumberRange.new(0.8, 1.5)
    particle.SpreadAngle   = Vector2.new(180, 180)
    particle.RotSpeed      = NumberRange.new(-45, 45); particle.Rotation = NumberRange.new(0, 360)
    ...
    auraCharConn = LocalPlayer.CharacterAdded:Connect(function()
        if auraEffect then auraEffect:Destroy(); auraEffect = nil end
        if auraConn   then auraConn:Disconnect(); auraConn = nil end
        task.wait(1); applyAura(auraName)
    end)
...
local function patchInstantBobber(enabled)
    ...
    local ok, cosmeticFolder = pcall(function() return workspace:WaitForChild("CosmeticFolder", 5) end)
    ...
    InstantBobberState.renderConn = RunService.RenderStepped:Connect(function()
        if not InstantBobberState.instantOverrideActive then return end
        local now = tick(); local cf = InstantBobberState.cosmeticFolder
...
local function createAndTPtoPlatform(targetPos, yOffset)
    ...
    if createdEventPlatform and createdEventPlatform.Parent then
        ...
        p.Size = Vector3.new(5,1,5); p.Position = desiredPos; p.Anchored = true
        p.Transparency = 1; p.CanCollide = true; p.Name = "MNA_EventPlatform"; p.Parent = workspace
                        ...
                        if props.Name == "Props" then
                            ...
                            if model and model.PrimaryPart then
                                ...
                                for _, loc in ipairs(cfg.Locations) do
                                    if (mPos - loc).Magnitude <= megCheckRadius then foundPos = mPos; break end
                                end
                            end
                        end
...
local enchantIdMap = {
    ...
    ["Gold Digger 1"]=4,["Leprechaun 1"]=5,["Leprechaun 2"]=6,["Mutation Hunter 1"]=7,
    ["Mutation Hunter 2"]=14,["Prismatic 1"]=13,["Reeler 1"]=2,["Stargazer 1"]=8,
    ["Stormhunter 1"]=11,["XPerienced 1"]=10,["SECRET Hunter"]=16,["Shark Hunter"]=20,
    ["Stargazer II"]=17,["Stormhunter II"]=19,["Leprechaun II"]=6,["Reeler II"]=21,
...
local function getEquippedRodName()
    ...
    pcall(function()
        if not PlayerData then return end
        local eq  = PlayerData:Get("EquippedItems"); if not eq then return end
        local inv = PlayerData:GetExpect("Inventory")
...
local function findEnchantStones()
    ...
    pcall(function()
        ...
        if not inv or not inv.Items then return end
        local tid = STONE_IDS[_G.SelectedStoneType or "Enchant Stones"]
        for _, item in ipairs(inv.Items) do if item.Id == tid then table.insert(stones, item) end end
    end)
    return stones
end
...
local function countHotbarButtons()
    ...
    pcall(function()
        ...
        count = 0
        for _, c in ipairs(dp:GetChildren()) do if c:IsA("ImageButton") then count = count + 1 end end
    end)
    return count
end
...
local function createRealtimePanel()
    ...
    gui.Parent = game:GetService("CoreGui"); realtimePanelGui = gui
    ...
    local st = Instance.new("UIStroke", main); st.Color = Color3.fromRGB(200, 50, 50); st.Thickness = 1.8
    ...
    titleLbl.Text = "MNA HUB — REALTIME STATS"; titleLbl.TextColor3 = Color3.fromRGB(200, 50, 50)
    ...
    local function makeStat()
        ...
        box.Size = UDim2.new(0,54,1,0); box.BackgroundColor3 = Color3.fromRGB(40, 0, 60)
        box.BackgroundTransparency = 0.2; box.BorderSizePixel = 0
        Instance.new("UICorner", box).CornerRadius = UDim.new(0, 8)
        local s2 = Instance.new("UIStroke", box); s2.Color = Color3.fromRGB(200, 50, 50); s2.Thickness = 1.2
    ...
    local pingLbl = makeStat(); local fpsLbl = makeStat()
    local notifLbl = makeStat(); local timeLbl = makeStat()
    ...
    local fpsConn = RunService.RenderStepped:Connect(function()
        ...
        if tick() - fpsLast >= 1 then fpsCurrent = fpsFrames; fpsFrames = 0; fpsLast = tick() end
    end)

    local function colorStat(lbl, v, y, r)
        lbl.TextColor3 = v >= r and Color3.fromRGB(255,80,80) or v >= y and Color3.fromRGB(255,210,50) or Color3.fromRGB(80,255,140)
    end
        ...
        while gui.Parent do
            ...
            pcall(function()
                ...
                if tn then
                    ...
                    if fr then for _, c in ipairs(fr:GetChildren()) do if c.Name == "Tile" or c:IsA("Frame") then notif = notif + 1 end end end
                end
            end)
            pingLbl.Text  = "PING\n"..ping.."ms"
            fpsLbl.Text   = "FPS\n"..fpsCurrent
            ...
            colorStat(pingLbl, ping, 120, 200); colorStat(fpsLbl, fpsCurrent, 40, 90); colorStat(notifLbl, notif, 8, 20)
            task.wait(1)
        end
        fpsConn:Disconnect()
    end)
...
-- ══════════════════════════════════════════
--    MNA HUB V11.3 — UI ENGINE
--    FIX: Ukuran 580x460 (WindUI style)
-- ══════════════════════════════════════════

-- Constants untuk ukuran (mudah diubah)
local WIN_W    = 580   -- lebar window  (WindUI: 580)
local WIN_H    = 460   -- tinggi window (WindUI: 460)
local SIDE_W   = 140   -- lebar sidebar
local TB_H     = 50    -- tinggi titlebar
local SB_H     = 28    -- tinggi statusbar
...
SG.Name = "MNAHUB_V113"; SG.ResetOnSpawn = false
...
WStroke.Color = Color3.fromRGB(160, 30, 30); WStroke.Thickness = 1.5

-- ── TITLE BAR ──
local TB = Instance.new("Frame", Win)
TB.Size = UDim2.new(1, 0, 0, TB_H)
TB.BackgroundColor3 = Color3.fromRGB(14, 8, 8); TB.BorderSizePixel = 0
Instance.new("UICorner", TB).CornerRadius = UDim.new(0, 12)
local TBFix = Instance.new("Frame", TB)
TBFix.Size = UDim2.new(1,0,0.5,0); TBFix.Position = UDim2.new(0,0,0.5,0)
TBFix.BackgroundColor3 = Color3.fromRGB(14,8,8); TBFix.BorderSizePixel = 0
local TBLine = Instance.new("Frame", TB)
TBLine.Size = UDim2.new(1,0,0,1); TBLine.Position = UDim2.new(0,0,1,-1)
TBLine.BackgroundColor3 = Color3.fromRGB(120,20,20); TBLine.BorderSizePixel = 0

local LogoF = Instance.new("Frame", TB)
LogoF.Size = UDim2.new(0,34,0,34); LogoF.Position = UDim2.new(0,9,0.5,-17)
LogoF.BackgroundColor3 = Color3.fromRGB(20,8,8); LogoF.BorderSizePixel = 0
Instance.new("UICorner", LogoF).CornerRadius = UDim.new(1,0)
local LogoStroke = Instance.new("UIStroke", LogoF)
LogoStroke.Color = Color3.fromRGB(200,50,50); LogoStroke.Thickness = 2
local LogoTxt = Instance.new("TextLabel", LogoF)
LogoTxt.Size = UDim2.new(1,0,1,0); LogoTxt.BackgroundTransparency = 1
LogoTxt.Text = "M"; LogoTxt.TextColor3 = Color3.fromRGB(220,80,80)
LogoTxt.TextSize = 16; LogoTxt.Font = Enum.Font.GothamBold

local TTitle = Instance.new("TextLabel", TB)
TTitle.Size = UDim2.new(0, 200, 0, 16); TTitle.Position = UDim2.new(0, 51, 0, 7)
TTitle.BackgroundTransparency = 1; TTitle.Text = "MNA HUB — V11.3 FREE"
TTitle.TextColor3 = Color3.fromRGB(240,180,180); TTitle.TextSize = 13
TTitle.Font = Enum.Font.GothamBold; TTitle.TextXAlignment = Enum.TextXAlignment.Left

local TSub = Instance.new("TextLabel", TB)
TSub.Size = UDim2.new(0,220,0,12); TSub.Position = UDim2.new(0,51,0,27)
TSub.BackgroundTransparency = 1; TSub.Text = "discord.gg/xjqJEnsY2  •  Fish It!"
TSub.TextColor3 = Color3.fromRGB(100,30,30); TSub.TextSize = 10
TSub.Font = Enum.Font.Gotham; TSub.TextXAlignment = Enum.TextXAlignment.Left
...
local function mkWinBtn(xOff, lbl, bg)
    ...
    b.Text = lbl; b.TextColor3 = Color3.fromRGB(200,120,120)
    b.TextSize = 11; b.Font = Enum.Font.GothamBold; b.BorderSizePixel = 0
    Instance.new("UICorner", b).CornerRadius = UDim.new(0,5)
    return b
end
local BMin = mkWinBtn(-52, "—")
local BCls = mkWinBtn(-28, "✕", Color3.fromRGB(110,18,18))

local Bdg = Instance.new("TextLabel", TB)
Bdg.Size = UDim2.new(0,52,0,18); Bdg.Position = UDim2.new(1,-108,0.5,-9)
Bdg.BackgroundColor3 = Color3.fromRGB(110,18,18); Bdg.Text = "FREE"
Bdg.TextColor3 = Color3.fromRGB(255,200,200); Bdg.TextSize = 10
Bdg.Font = Enum.Font.GothamBold; Bdg.BorderSizePixel = 0
...
BCls.MouseButton1Click:Connect(function()
    ...
    task.wait(0.25); SG.Enabled = false
end)
BMin.MouseButton1Click:Connect(function()
    minimized = not minimized
    TweenService:Create(Win, TweenInfo.new(0.2, Enum.EasingStyle.Quart), {
        Size = minimized and UDim2.new(0, WIN_W, 0, TB_H) or UDim2.new(0, WIN_W, 0, WIN_H)
    }):Play()
end)

-- ── CONTENT AREA ──
local Cont = Instance.new("Frame", Win)
Cont.Size = UDim2.new(1,-16, 1, -(TB_H + SB_H + 10))
Cont.Position = UDim2.new(0,8, 0, TB_H + 4)
Cont.BackgroundTransparency = 1; Cont.BorderSizePixel = 0

local SBar = Instance.new("ScrollingFrame", Cont)
SBar.Size = UDim2.new(0, SIDE_W, 1, 0)
SBar.BackgroundTransparency = 1; SBar.BorderSizePixel = 0
SBar.ScrollBarThickness = 2; SBar.ScrollBarImageColor3 = Color3.fromRGB(120,20,20)
SBar.CanvasSize = UDim2.new(0,0,0,0); SBar.AutomaticCanvasSize = Enum.AutomaticSize.Y
Instance.new("UIListLayout", SBar).Padding = UDim.new(0, 2)

local SDivider = Instance.new("Frame", Cont)
SDivider.Size = UDim2.new(0,1,1,0); SDivider.Position = UDim2.new(0, SIDE_W+2, 0, 0)
SDivider.BackgroundColor3 = Color3.fromRGB(80,15,15); SDivider.BorderSizePixel = 0

local RPan = Instance.new("ScrollingFrame", Cont)
RPan.Size = UDim2.new(1, -(SIDE_W+10), 1, 0)
RPan.Position = UDim2.new(0, SIDE_W+8, 0, 0)
RPan.BackgroundTransparency = 1; RPan.BorderSizePixel = 0
RPan.ScrollBarThickness = 3; RPan.ScrollBarImageColor3 = Color3.fromRGB(160,30,30)
RPan.CanvasSize = UDim2.new(0,0,0,0); RPan.AutomaticCanvasSize = Enum.AutomaticSize.Y
local RPLayout = Instance.new("UIListLayout", RPan); RPLayout.Padding = UDim.new(0, 5)
local RPPad = Instance.new("UIPadding", RPan)
RPPad.PaddingTop = UDim.new(0,4); RPPad.PaddingBottom = UDim.new(0,8); RPPad.PaddingRight = UDim.new(0,4)

-- ── STATUS BAR ──
local UBar = Instance.new("Frame", Win)
UBar.Size = UDim2.new(1,-16, 0, SB_H)
UBar.Position = UDim2.new(0, 8, 1, -(SB_H + 4))
UBar.BackgroundColor3 = Color3.fromRGB(12,6,6); UBar.BorderSizePixel = 0
...
UName.Size = UDim2.new(0,200,1,0); UName.Position = UDim2.new(0,10,0,0)
UName.BackgroundTransparency = 1
UName.Text = "👑  " .. LocalPlayer.DisplayName .. "  (@" .. LocalPlayer.Name:lower() .. ")"
UName.TextColor3 = Color3.fromRGB(200,140,140); UName.TextSize = 10
UName.Font = Enum.Font.Gotham; UName.TextXAlignment = Enum.TextXAlignment.Left

local UStatus = Instance.new("TextLabel", UBar)
UStatus.Size = UDim2.new(0,150,1,0); UStatus.Position = UDim2.new(1,-155,0,0)
UStatus.BackgroundTransparency = 1; UStatus.Text = "● Ready"
UStatus.TextColor3 = Color3.fromRGB(120,40,40); UStatus.TextSize = 10
UStatus.Font = Enum.Font.Gotham; UStatus.TextXAlignment = Enum.TextXAlignment.Right
...
local function sideSection(txt)
    ...
    l.Text = txt; l.TextColor3 = Color3.fromRGB(100,25,25)
...
local function sideItem(id, icon, lbl)
    ...
    iL.Size = UDim2.new(0,18,1,0); iL.Position = UDim2.new(0,5,0,0)
    iL.BackgroundTransparency = 1; iL.Text = icon; iL.TextSize = 13; iL.Font = Enum.Font.Gotham
    ...
    nL.TextColor3 = Color3.fromRGB(130,60,60); nL.TextSize = 11
    nL.Font = Enum.Font.Gotham; nL.TextXAlignment = Enum.TextXAlignment.Left

    local panel = Instance.new("Frame", RPan)
    panel.Size = UDim2.new(1,0,0,0); panel.BackgroundTransparency = 1
    ...
    allPanels[id] = panel; allSBtns[id] = { btn = btn, nL = nL }
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        for _, sb in pairs(allSBtns) do
            ...
            sb.nL.TextColor3 = Color3.fromRGB(130,60,60); sb.nL.Font = Enum.Font.Gotham
        end
        TweenService:Create(btn, TweenInfo.new(0.12), {
            BackgroundTransparency = 0, BackgroundColor3 = Color3.fromRGB(40,8,8)
        }):Play()
        nL.TextColor3 = Color3.fromRGB(240,160,160); nL.Font = Enum.Font.GothamBold
        panel.Visible = true; RPan.CanvasPosition = Vector2.new(0, 0)
    end)
    return panel
end
...
local function addInfo(parent, txt)
    ...
    f.Size = UDim2.new(1,0,0,24); f.BackgroundColor3 = Color3.fromRGB(12,5,5); f.BorderSizePixel = 0
    Instance.new("UICorner", f).CornerRadius = UDim.new(0, 6)
    local l = Instance.new("TextLabel", f)
    l.Size = UDim2.new(1,-14,1,0); l.Position = UDim2.new(0,8,0,0)
    l.BackgroundTransparency = 1; l.Text = txt; l.TextColor3 = Color3.fromRGB(120,50,50)
    l.TextSize = 10; l.Font = Enum.Font.Gotham; l.TextXAlignment = Enum.TextXAlignment.Left; l.TextWrapped = true
end

local function addSection(parent, txt)
    local f = Instance.new("Frame", parent)
    f.Size = UDim2.new(1,0,0,20); f.BackgroundColor3 = Color3.fromRGB(14,6,6); f.BorderSizePixel = 0
    Instance.new("UICorner", f).CornerRadius = UDim.new(0, 5)
    local l = Instance.new("TextLabel", f)
    l.Size = UDim2.new(1,-14,1,0); l.Position = UDim2.new(0,8,0,0)
    l.BackgroundTransparency = 1; l.Text = txt; l.TextColor3 = Color3.fromRGB(200,70,70)
...
local function addToggle(parent, lbl, getVal, setVal, cb)
    ...
    row.Size = UDim2.new(1,0,0,36); row.BackgroundColor3 = Color3.fromRGB(10,5,5); row.BorderSizePixel = 0
    Instance.new("UICorner", row).CornerRadius = UDim.new(0, 8)
    Instance.new("UIStroke", row).Color = Color3.fromRGB(50,12,12)

    local t = Instance.new("TextLabel", row)
    t.Size = UDim2.new(1,-54,1,0); t.Position = UDim2.new(0,10,0,0)
    t.BackgroundTransparency = 1; t.Text = lbl
    t.TextColor3 = Color3.fromRGB(190,130,130); t.TextSize = 11
    t.Font = Enum.Font.Gotham; t.TextXAlignment = Enum.TextXAlignment.Left

    local pill = Instance.new("TextButton", row)
    pill.Size = UDim2.new(0,38,0,19); pill.Position = UDim2.new(1,-45,0.5,-9.5)
    local initOn = getVal()
    ...
    circle.Position = initOn and UDim2.new(1,-16,0.5,-6.5) or UDim2.new(0,3,0.5,-6.5)
    circle.BackgroundColor3 = Color3.fromRGB(255,255,255); circle.BorderSizePixel = 0
    Instance.new("UICorner", circle).CornerRadius = UDim.new(1, 0)

    local function doToggle()
        setVal(not getVal()); local on = getVal()
        ...
        TweenService:Create(circle, TweenInfo.new(0.15), { Position = on and UDim2.new(1,-16,0.5,-6.5) or UDim2.new(0,3,0.5,-6.5) }):Play()
...
local function addSlider(parent, lbl, mn, mx, def, step, cb)
    ...
    sf.Size = UDim2.new(1,0,0,48); sf.BackgroundColor3 = Color3.fromRGB(10,5,5); sf.BorderSizePixel = 0
    ...
    sl.Size = UDim2.new(1,-14,0,16); sl.Position = UDim2.new(0,9,0,5)
    sl.BackgroundTransparency = 1; sl.Text = lbl .. "   " .. tostring(def)
    sl.TextColor3 = Color3.fromRGB(200,130,130); sl.TextSize = 11
    sl.Font = Enum.Font.Gotham; sl.TextXAlignment = Enum.TextXAlignment.Left

    local track = Instance.new("Frame", sf)
    track.Size = UDim2.new(1,-18,0,5); track.Position = UDim2.new(0,9,0,28)
    track.BackgroundColor3 = Color3.fromRGB(22,8,8); track.BorderSizePixel = 0
    Instance.new("UICorner", track).CornerRadius = UDim.new(1, 0)

    local pct  = math.clamp((def - mn) / (mx - mn), 0, 1)
    local fill = Instance.new("Frame", track)
    fill.Size = UDim2.new(pct,0,1,0); fill.BackgroundColor3 = Color3.fromRGB(160,25,25); fill.BorderSizePixel = 0
    Instance.new("UICorner", fill).CornerRadius = UDim.new(1, 0)

    local knob = Instance.new("TextButton", track)
    knob.Size = UDim2.new(0,12,0,12); knob.Position = UDim2.new(pct,-6,0.5,-6)
    knob.BackgroundColor3 = Color3.fromRGB(220,80,80); knob.Text = ""; knob.BorderSizePixel = 0
...
local function addInput(parent, lbl, placeholder, cb)
    local f = Instance.new("Frame", parent)
    f.Size = UDim2.new(1,0,0,48); f.BackgroundColor3 = Color3.fromRGB(10,5,5); f.BorderSizePixel = 0
    Instance.new("UICorner", f).CornerRadius = UDim.new(0, 8)
    Instance.new("UIStroke", f).Color = Color3.fromRGB(50,12,12)

    local lL = Instance.new("TextLabel", f)
    lL.Size = UDim2.new(1,-10,0,16); lL.Position = UDim2.new(0,8,0,4)
    lL.BackgroundTransparency = 1; lL.Text = lbl
    lL.TextColor3 = Color3.fromRGB(180,100,100); lL.TextSize = 10
    ...
    box.BackgroundColor3 = Color3.fromRGB(16,8,8); box.BorderSizePixel = 0
    box.Text = ""; box.PlaceholderText = placeholder or "..."
    box.PlaceholderColor3 = Color3.fromRGB(80,30,30)
    box.TextColor3 = Color3.fromRGB(220,160,160); box.TextSize = 11
...
local function addButton(parent, lbl, col, cb)
    ...
    b.Size = UDim2.new(1,0,0,30); b.BackgroundColor3 = col or Color3.fromRGB(110,18,18)
    b.Text = lbl; b.TextColor3 = Color3.fromRGB(255,200,200)
    b.TextSize = 11.5; b.Font = Enum.Font.GothamBold; b.BorderSizePixel = 0
...
local function addDropdown(parent, lbl, values, defaultVal, cb)
    ...
    local lL = Instance.new("TextLabel", f)
    lL.Size = UDim2.new(1,-10,0,16); lL.Position = UDim2.new(0,8,0,3)
    lL.BackgroundTransparency = 1; lL.Text = lbl
    lL.TextColor3 = Color3.fromRGB(180,100,100); lL.TextSize = 10
    lL.Font = Enum.Font.Gotham; lL.TextXAlignment = Enum.TextXAlignment.Left
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        listFrame.Size = UDim2.new(0, absSize.X, 0, math.min(#values * 24, 160))
        listFrame.BackgroundColor3 = Color3.fromRGB(14,6,6); listFrame.BorderSizePixel = 0; listFrame.ZIndex = 20
        Instance.new("UICorner", listFrame).CornerRadius = UDim.new(0, 6)
        Instance.new("UIStroke", listFrame).Color = Color3.fromRGB(80,15,15)

        local sf2 = Instance.new("ScrollingFrame", listFrame)
        sf2.Size = UDim2.new(1,0,1,0); sf2.BackgroundTransparency = 1; sf2.BorderSizePixel = 0
        sf2.ScrollBarThickness = 2; sf2.ScrollBarImageColor3 = Color3.fromRGB(120,20,20)
        sf2.CanvasSize = UDim2.new(0,0,0,0); sf2.AutomaticCanvasSize = Enum.AutomaticSize.Y; sf2.ZIndex = 20
        Instance.new("UIListLayout", sf2).Padding = UDim.new(0, 1)

        for _, v in ipairs(values) do
            local item = Instance.new("TextButton", sf2)
            item.Size = UDim2.new(1,0,0,23)
            item.BackgroundColor3 = tostring(v) == tostring(current) and Color3.fromRGB(40,10,10) or Color3.fromRGB(14,6,6)
            item.Text = "  " .. tostring(v); item.TextColor3 = Color3.fromRGB(220,150,150)
            item.TextSize = 11; item.Font = Enum.Font.Gotham
            item.TextXAlignment = Enum.TextXAlignment.Left; item.BorderSizePixel = 0; item.ZIndex = 21
...
addInfo(pInfo, "MNA HUB V11.4 — gg/MNATeam! | FreeScript")
addInfo(pInfo, "  [RightShift] = Toggle GUI buka/tutup")
addInfo(pInfo, "  FREE NOT FOR SELLL!!!")
...
addSection(pFish, "Blatant 3N")
addDropdown(pFish, "Template Rod", {
    "1. 3 NOTIF (BETA) (2.998s)", "2. Diamond / Element (3.7s)",
    "3. (2) NOTIF IS REAL (2.890s)", "4. GostFin / Bambu (3.7s)", "5. Ares/Angler/Astral (4.8s)"
}, "2. Diamond / Element (3.7s)", function(v)
    local map = {
        ["1. 3 NOTIF (BETA)(2.998s)"]=2.998, ["2. Diamond / Element (3.7s)"]=3.7,
        ["3. (2) NOTIF IS REAL (2.890s)"]=2.890, ["4. Gostfin / Bambu (3.7s)"]=3.7, ["5. Ares/Angler/Astral (4.8s)"]=4.8,
    }
...
addInput(pFish, "Complete Delay (detik)", tostring(Config.UB.Settings.CompleteDelay), function(t)
    local n = tonumber(t)
    if n and n >= 1 then Config.UB.Settings.CompleteDelay = n; NotifySuccess("Delay", "Set: "..n.."s")
    else NotifyError("Delay", "Minimal 1 detik!") end
end)
...
addToggle(pFish, "Blatant 3N",
    ...
    function(v) if v then Config.amblatant = false end end,
    function(v)
...
addToggle(pFish, "  amBlantant",
    ...
    function(v)
        ...
        HookRemote("RE/ObtainedNewFishNotification","FishNotif")
        needCast = true
        if v then onToggleUB(true); setStatus("amBlantat AKTIF ✨", true)
        else Config.amblatant = false; setStatus("Ready", false); NotifyWarning("amBlantat","Dimatikan. UB tetap jalan.") end
    end
)
addToggle(pFish, "  Random Cast ",
...
addSection(pFish, "UB amblantant")
addSlider(pFish, "Notif amBlantat", 1, 10, 3, 1, function(v)
    Config.NotifCount = v; NotifyInfo("amBlantat Notif", "Jumlah: "..v.."x per catch")
end)
addSlider(pFish, "Delay Notif (x0.01s)", 0, 100, 10, 1, function(v)
    Config.NotifDelay = v / 100; NotifyInfo("amBlantat Delay", "Delay: "..Config.NotifDelay.."s")
end)
...
addToggle(pFish, "  Instant Bobber",
    ...
    function(v) patchInstantBobber(v); NotifyInfo("Instant Bobber", v and "Aktif!" or "Dimatikan.") end
)
...
}, "Eclipse", function(v) Config.SelectedSkinId = v; SkinAnimation.Switch(v) end)
addToggle(pFish, "  Enable Skin Animation",
    function() return Config.SkinAnimEnabled end,
    function(v) Config.SkinAnimEnabled = v end,
    function(v)
        if v then SkinAnimation.Switch(Config.SelectedSkinId); SkinAnimation.Enable(); NotifySuccess("Skin Anim", Config.SelectedSkinId.." aktif!")
        else SkinAnimation.Disable(); NotifyWarning("Skin Anim","Dimatikan.") end
    end
)
...
addToggle(pFish, "  Legit Auto Catch",
    ...
    function(v)
        if v then equipRod(); CallRemote(Events.UpdateAutoFishing,true); NotifySuccess("Legit","Aktif!")
        else CallRemote(Events.UpdateAutoFishing,false); NotifyWarning("Legit","Dimatikan.") end
    end
)
addInput(pFish, "Catch Delay (detik)", "0.7", function(t)
    local n = tonumber(t)
    if n and n >= 0 then Config.CatchDelay = n; NotifySuccess("Catch Delay","Set: "..n.."s") end
end)
addToggle(pFish, "  Perfection Enchant",
    function() return Config.autoFishing end,
    function(v) Config.autoFishing = v end,
    function(v) CallRemote(Events.UpdateAutoFishing,v); if v then NotifySuccess("Perfection","Aktif!") else NotifyWarning("Perfection","Dimatikan.") end end
)

addSection(pFish, "Auto Sell")
addDropdown(pFish, "Metode Sell", {"Delay","Count"}, "Delay", function(v)
    Config.AutoSellMethod = v; if Config.AutoSellState then RunAutoSellLoop() end
end)
...
addToggle(pFish, "  Enable Auto Sell",
    ...
    function(v)
        if v then RunAutoSellLoop(); NotifySuccess("Auto Sell","Aktif! Mode: "..Config.AutoSellMethod)
...
addButton(pFish, "  Sell All Now", Color3.fromRGB(80,15,15), function()
    if Events.sell then pcall(function() Events.sell:InvokeServer({}) end); NotifySuccess("Sell","Semua ikan dijual!")
    else NotifyError("Sell","Remote tidak ditemukan!") end
end)

addSection(pFish, "Auto Favorite")
addDropdown(pFish, "Filter Rarity", {"Common","Uncommon","Rare","Epic","Legendary","Mythic","SECRET"}, "Legendary", function(v)
...
addSlider(pPlayer, "Jump Power", 50, 500, 50, 10, function(v)
    local char = LocalPlayer.Character
    if char then local hum = char:FindFirstChildOfClass("Humanoid"); if hum then hum.UseJumpPower = true; hum.JumpPower = v end end
end)
...
-- ── MAIN ──
local pMain = sideItem("main", "", "Main")
addSection(pMain, "Enchant Status")
local enchStatFrame = Instance.new("Frame", pMain)
enchStatFrame.Size = UDim2.new(1,0,0,46); enchStatFrame.BackgroundColor3 = Color3.fromRGB(10,5,5); enchStatFrame.BorderSizePixel = 0
...
enchStatLbl.Size = UDim2.new(1,-12,1,0); enchStatLbl.Position = UDim2.new(0,7,0,0)
enchStatLbl.BackgroundTransparency = 1; enchStatLbl.Text = "Rod: - | Enchant: - | Stone: 0"
enchStatLbl.TextColor3 = Color3.fromRGB(180,110,110); enchStatLbl.TextSize = 10
enchStatLbl.Font = Enum.Font.Gotham; enchStatLbl.TextXAlignment = Enum.TextXAlignment.Left; enchStatLbl.TextWrapped = true
...
task.spawn(function()
    ...
    while true do
        task.wait(2)
        pcall(function()
            local s = getEnchantStoneCount(); local r = getEquippedRodName()
            local eid = getCurrentRodEnchant(); local en = "None"
            if eid then for n, id in pairs(enchantIdMap) do if id == eid then en = n; break end end end
...
addDropdown(pMain, "Stone Type", {"Enchant Stones","Evolved Enchant Stone"}, "Enchant Stones", function(v) _G.SelectedStoneType = v end)
...
}, "Big Hunter 1", function(v) _G.TargetEnchantBasic = v end)
            ...
            task.spawn(function()
                while _G.AutoEnchant do
                    pcall(function()
                        ...
                        local stoneUUID = nil
                        for _, item in ipairs(inv.Items) do if item.Id == targetId then stoneUUID = item.UUID; break end end
...
addButton(pMain, "  Teleport ke Altar", Color3.fromRGB(50,10,10), function()
    local hrp = getHRP()
    if hrp then hrp.CFrame = CFrame.new(3234.837,-1302.855,1398.391); NotifySuccess("Teleport","Berhasil ke Altar!") end
end)
addButton(pMain, "  Double Enchant (Sekali)", Color3.fromRGB(40,10,40), function()
    task.spawn(function()
        ...
        task.wait(1.2)
        local slot = countHotbarButtons() - 1; if slot < 1 then slot = 1 end
...
addButton(pMain, "  Fix Rod", Color3.fromRGB(30,8,8), function()
    if Events.CancelFishing then pcall(function() CallRemote(Events.CancelFishing) end); NotifySuccess("Fix Rod","Rod di-reset!")
    elseif Config.UB.Remotes.CancelFishingInputs then pcall(function() CallRemote(Config.UB.Remotes.CancelFishingInputs) end); NotifySuccess("Fix Rod","Rod di-reset!")
...
addButton(pMain, "  Consume Crystal Now", Color3.fromRGB(10,30,50), function()
    if Events.ConsumeCaveCrystal then pcall(function() Events.ConsumeCaveCrystal:InvokeServer() end); task.wait(1.5); equipRod(); NotifySuccess("Crystal","Dikonsumsi!")
...
addDropdown(pMain, "Pilih Totem", {"Pilih Totem","Luck Totem","Mutation Totem","Shiny Totem","Love Totem"}, "Pilih Totem", function(v)
    local m = {["Pilih Totem"]=0,["Luck Totem"]=1,["Mutation Totem"]=2,["Shiny Totem"]=3,["Love Totem"]=5}
    Config.SelectedTotemID = m[v] or 0; NotifyInfo("Totem","Dipilih: "..v)
end)
                        ...
                        pcall(function()
                            ...
                            if inv and inv.Totems then
                                for _, item in ipairs(inv.Totems) do
                                    if Config.SelectedTotemID == 0 or item.Id == Config.SelectedTotemID then totemUUID = item.UUID; break end
                                end
                            end
                        end)
                        if totemUUID and Events.SpawnTotem then
                            pcall(function() Events.SpawnTotem:FireServer(totemUUID) end)
                            task.wait(3); equipRod(); NotifySuccess("Totem","Spawn! Cooldown 1 jam.")
                        end
                    end)
                    task.wait(3600)
                end
...
addDropdown(pTp, "Pilih Lokasi", locList, locList[1], function(v) selectedLoc = v end)
addButton(pTp, "  Teleport ke Lokasi!", Color3.fromRGB(50,10,10), function()
    teleportTo(selectedLoc); NotifySuccess("Teleport","Berhasil ke "..selectedLoc)
end)
...
addDropdown(pTp, "Pilih Player", playerList, playerList[1], function(v) selectedPlayer = v end)
addButton(pTp, "  Teleport ke Player", Color3.fromRGB(40,10,10), function()
    if not selectedPlayer or selectedPlayer == "-- Tidak ada --" then NotifyError("Error","Pilih player dulu!"); return end
    local target = Players:FindFirstChild(selectedPlayer)
    if target and target.Character then
        local tHRP = target.Character:FindFirstChild("HumanoidRootPart"); local hrp = getHRP()
        if hrp and tHRP then hrp.CFrame = CFrame.new(tHRP.Position + Vector3.new(0,3,0)); NotifySuccess("Teleport","Berhasil ke "..selectedPlayer) end
    else NotifyError("Error","Character tidak ditemukan!") end
end)
                    ...
                    pcall(function()
                        ...
                        if zones then
                            ...
                            if den and not hasTP then
                                local hrp = getHRP()
                                if hrp then hrp.CFrame = CFrame.new(3474.053,-287.775,3472.634); hasTP = true; NotifySuccess("Leviathan","TP ke Den!") end
...
addDropdown(pTp, "Pilih Event", eventTPNames, eventTPNames[1], function(v) selectedAutoEvents = {v} end)
addToggle(pTp, "  Auto Event Teleport",
    function() return autoEventTPEnabled end,
    function(v) autoEventTPEnabled = v end,
    function(v)
        if v then
            if #selectedAutoEvents == 0 then NotifyError("Auto Event TP","Pilih event dulu!"); autoEventTPEnabled = false; return end
            if autoEventThread then pcall(function() task.cancel(autoEventThread) end) end
            autoEventThread = task.spawn(runAutoEventTP); NotifySuccess("Auto Event TP","Aktif!")
        else
            destroyEventPlatform()
...
addButton(pEaster, "  TP ke Easter Island", Color3.fromRGB(40,25,5), function()
    teleportTo("Easter Island"); NotifySuccess("Teleport","Berhasil ke Easter Island!")
end)
addButton(pEaster, "  TP ke Easter Cove", Color3.fromRGB(5,25,40), function()
    teleportTo("Easter Cove"); NotifySuccess("Teleport","Berhasil ke Easter Cove!")
end)
...
addToggle(pEaster, "  Auto Egg Hunt",
    ...
    function(v)
        if v then startAutoEggHunt(); NotifySuccess("Auto Egg Hunt","Aktif!")
        else if Tasks.EggHuntTask then pcall(function() task.cancel(Tasks.EggHuntTask) end) end; NotifyWarning("Auto Egg Hunt","Dimatikan.") end
    end
)
...
addToggle(pEaster, "  Auto Buy Egg",
    ...
    function(v)
        ...
        else if Tasks.BuyEggTask then pcall(function() task.cancel(Tasks.BuyEggTask) end) end; NotifyWarning("Auto Buy Egg","Dimatikan.") end
    end
)
addButton(pEaster, "  Buy Egg Once", Color3.fromRGB(40,20,5), function()
    ...
    if Events.BuyEgg then pcall(function() Events.BuyEgg:InvokeServer(Config.SelectedEggId) end); NotifySuccess("Buy Egg","Egg dibeli!")
...
addToggle(pEaster, "  Auto Event",
    ...
    function(v)
        ...
        else if Tasks.AutoEventTask then pcall(function() task.cancel(Tasks.AutoEventTask) end) end; NotifyWarning("Auto Event","Dimatikan.") end
    end
)
...
addDropdown(pShop, "Pilih Weather", wxNames2, wxNames2[1], function(v) selectedWeather = v end)
addButton(pShop, "  Buy Selected Weather", Color3.fromRGB(10,20,50), function()
    ...
    if key then pcall(function() Events.BuyWeather:InvokeServer(key) end); NotifySuccess("Weather","Purchased: "..selectedWeather) end
end)
...
addDropdown(pCosmetic, "Pilih Aura", auraNameList, "None", function(v) selectedAuraName = v end)
addButton(pCosmetic, "🌟  Apply Aura", Color3.fromRGB(50,25,5), function() applyAura(selectedAuraName) end)
addButton(pCosmetic, "✕  Remove Aura", Color3.fromRGB(40,8,8), function() applyAura("None") end)

-- ── MISC ──
local pMisc = sideItem("misc", "", "Misc")
addSection(pMisc, "Visual & Performance")
addToggle(pMisc, "  No Animation",
    function() return false end, function(v) end,
    function(v)
        local char = LocalPlayer.Character
        if char then
            local hum = char:FindFirstChildOfClass("Humanoid")
            ...
            if anim then
                if v then
                    ...
                    anim.AnimationPlayed:Connect(function(t) if v then t:Stop(0) end end)
                end
            end
        end
...
addToggle(pMisc, "  FPS Booster",
    ...
    function(v)
        if v then
            ...
            local L = game:GetService("Lighting"); L.GlobalShadows = false; L.FogEnd = 1e10
...
addToggle(pMisc, "  Ping Panel (pojok kanan atas)",
    function() return pingGui ~= nil end, function(v) end,
    function(v)
...
addToggle(pMisc, "  Realtime Stats Panel",
    function() return realtimePanelGui ~= nil end, function(v) end,
    function(v)
...
addInput(pMisc, "Webhook URL", "https://discord.com/api/webhooks/...", function(t)
    ...
    Config.CustomWebhookUrl = t; NotifySuccess("Webhook","URL disimpan!")
end)
