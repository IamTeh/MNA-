--[[
    MNA HUB V11.4  |  FREE NOT SELL
    UI  : WindUI (Crimson Theme)
    Fix : SkinDatabase "Katana" key mismatch, typo label, timeLbl dead-code,
          realtime panel sessionStart leak, semua label tab dirapikan

    [Ultra Blatant 3N]
      - Setiap cast → notif masuk _G.NotifQueue
      - Processor: tampil maks 2 notif serentak di layar
      - Durasi notif diperpanjang agar selalu 2 notif terlihat

    [amBlantat]
      - Catch REAL dari server → simpan snapshot
      - Replay: FishCaught × 1 + CaughtVisual × 1 + Notif × N
      - TIDAK pakai NotifQueue — sistem terpisah
]]
...
print("[MNA HUB] Remotes total: "..#net:GetChildren())

local function GetServerRemote(targetName)
    local all = net:GetChildren()
    for i, remote in ipairs(all) do
        if remote.Name == targetName then return all[i+1] end
    end
    return nil
end
...
local Config = {
    ...
    UB={
        ...
        Remotes={}, Stats={ castCount=0, startTime=0 }
    },
    ...
    HookNotif=false, NotifDelay=0.1, NotifCount=3, UBNotifDurationMult=2.0,
    WalkOnWater=false, HideNameTag=false, DisableObtained=false,
    AutoEggHunt=false, AutoBuyEgg=false, SelectedEggId="", AutoEvent=false,
    InstantBobber=false, SkinAnimEnabled=false,
    -- FIX: default skin sesuai list yang ada
...
local function deepCopyArr(t)
    ...
    for i,v in ipairs(t) do
        if type(v)=="table" then
            local c={}; for k,val in pairs(v) do c[k]=val end; out[i]=c
        else out[i]=v end
    end
    return out
end
...
local function loadRemotes()
    ...
    for _,r in ipairs(list) do
        ...
        Events[r.key] = remote
        if remote then loaded=loaded+1 else failed=failed+1 end
    end
    ...
    while true do
        ...
        if Config.UB.Active and not Config.amblatant then
            if #_G.NotifQueue > 0 and _G.NotifActive < MAX_NOTIF_ONSCREEN then
                ...
                local xr   = getNotifRemote()
                if xr and type(args)=="table" and #args > 0 then
                    _G.NotifActive = _G.NotifActive + 1
                    FireLocalEvent(xr, unpack(args))
...
local function replayAmblatantNotif()
    task.spawn(function()
        ...
        if xr_caught and #lastValidFishCaught>0 then FireLocalEvent(xr_caught,unpack(lastValidFishCaught)) end
        task.wait(0.03)
        if xr_visual and #lastValidCaughtVisual>0 then FireLocalEvent(xr_visual,unpack(lastValidCaughtVisual)) end
        task.wait(0.03)
        ...
        local ok,err = pcall(function()
            ...
            if not skip then
                ...
                if Config.amblatant then
                    ...
                    if #lastValidFishNotif>0 then table.insert(_G.NotifQueue,deepCopyArr(lastValidFishNotif)) end
                    if isCaught then
                        isCaught = false
                        if #(_G.SavedData.FishNotif or {})>0 then lastValidFishNotif=deepCopyArr(_G.SavedData.FishNotif) end
                    end
...
local function onToggleUB(value)
    if value then Config.HookNotif=true; equipRod(); task.wait(0.5); UB_start()
    else UB_stop(); Config.HookNotif=false end
end
...
task.spawn(function()
    while true do
        ...
        and os.clock()-lastTimeFishCaught>=20 and blatantFishCycleCount>1 then
    ...
    Tasks.AutoSellThread = task.spawn(function()
        while Config.AutoSellState do
            ...
            elseif Config.AutoSellMethod=="Count" then
                ...
                while Config.AutoSellState and (saveCount-startCount)<Config.AutoSellValue do task.wait(1) end
            ...
            if hrp.Position.Y < waterY then
                hrp.CFrame = CFrame.new(hrp.Position.X, waterY, hrp.Position.Z)
                    * CFrame.fromEulerAnglesXYZ(0,
                        math.atan2(hrp.CFrame.LookVector.X, hrp.CFrame.LookVector.Z), 0)
            end
        end)
...
-- ============================================================
--  DISABLE OBTAINED  (fix getconnections)
-- ============================================================
...
local function setDisableObtained(enabled)
    ...
    if enabled then
        ...
        if xr then
            pcall(function()
                for _,conn in pairs(getconnections(xr.OnClientEvent)) do
                    if conn.Function then
                        conn:Disable()
                        table.insert(obtainedDisabledConns,{ conn=conn })
                    end
                end
            end)
        end
        ...
        for _,entry in ipairs(obtainedDisabledConns) do
            pcall(function() if entry.conn and entry.conn.Enable then entry.conn:Enable() end end)
        end
                            ...
                            if hrp then
                                ...
                                if pos then
                                    ...
                                    for _,remote in pairs(net:GetChildren()) do
                                        if remote.Name:lower():find("event") or remote.Name:lower():find("interact") then
                                            pcall(function()
...
-- ============================================================
--  AURA  (fix memory leak)
-- ============================================================
local auraEffect,auraConn,auraCharConn = nil,nil,nil
...
local function patchInstantBobber(enabled)
    if not enabled then
        IBS.instantOverrideActive=false
        if IBS.activeBaitsByUserId then table.clear(IBS.activeBaitsByUserId) end
        if IBS.renderConn then IBS.renderConn:Disconnect(); IBS.renderConn=nil end
        return
    end
    IBS.instantOverrideActive=true
    IBS.activeBaitsByUserId=IBS.activeBaitsByUserId or {}
    table.clear(IBS.activeBaitsByUserId)
    if IBS.instantOverrideSetupDone then return end
    IBS.instantOverrideSetupDone=true

    local ok,cf2 = pcall(function() return workspace:WaitForChild("CosmeticFolder",5) end)
    if not ok or not cf2 then IBS.instantOverrideSetupDone=false; IBS.instantOverrideActive=false; return end
    IBS.cosmeticFolder=cf2
    ...
    or not baitDestroyed  or not baitDestroyed:IsA("RemoteEvent") then
        IBS.instantOverrideSetupDone=false; IBS.instantOverrideActive=false; return
    end

    IBS.baitCastConn = baitCastVisual.OnClientEvent:Connect(function(player,data)
        if not IBS.instantOverrideActive or not player or not player.UserId then return end
        if not data or not data.CastPosition or typeof(data.CastPosition)~="Vector3" then return end
        IBS.activeBaitsByUserId[player.UserId]={ pivot=CFrame.new(data.CastPosition), expiresAt=tick()+1.5 }
    end)
    IBS.baitDestroyedConn = baitDestroyed.OnClientEvent:Connect(function(player)
        if not IBS.instantOverrideActive or not player or not player.UserId then return end
        IBS.activeBaitsByUserId[player.UserId]=nil
    end)
    IBS.renderConn = RunService.RenderStepped:Connect(function()
        if not IBS.instantOverrideActive then return end
        local now=tick(); local cf3=IBS.cosmeticFolder; if not cf3 then return end
        for userId,entry in pairs(IBS.activeBaitsByUserId) do
            if now>entry.expiresAt then IBS.activeBaitsByUserId[userId]=nil
            else
                local model=cf3:FindFirstChild(tostring(userId))
...
--  FIX: key "Katana" dihapus → semua key sekarang cocok dengan ubSkinNames
-- ============================================================
local SkinAnimation = (function()
    ...
    -- FIX: key "Katana" diganti "Eclipse" agar cocok dengan dropdown list
    local SkinDatabase = {
...
local eventTPData = {
    ["Worm Hunt"]={
        TargetName="Model",
        Locations={ Vector3.new(2190.85,-1.4,97.575), Vector3.new(-2450.679,-1.4,139.731),
    ...
    ["Shark Hunt"]={
        ...
        Locations={ Vector3.new(1.65,-1.35,2095.725), Vector3.new(1369.95,-1.35,930.125),
...
local function createAndTPtoPlatform(targetPos,yOffset)
    ...
    if createdEventPlatform and createdEventPlatform.Parent then
        ...
        p.Name="MNA_EventPlatform"; p.Parent=workspace
...
--  FIX: timeLbl dihapus (tidak ada di makeStat call sebelumnya),
--       sessionStart & fmtTime masih ada jadi sekarang dipakai kembali
-- ============================================================
local realtimePanelGui,realtimePanelTask = nil,nil
...
local function createRealtimePanel()
    ...
    main.Size=UDim2.new(0,270,0,90); main.Position=UDim2.new(0.5,-135,1,-100)
    main.BackgroundColor3=Color3.fromRGB(14,0,28); main.BackgroundTransparency=0.2
    ...
    header.Size=UDim2.new(1,0,0,26); header.BackgroundColor3=Color3.fromRGB(200,50,50)
    ...
    titleLbl.Text="MNA HUB — REALTIME"; titleLbl.TextColor3=Color3.fromRGB(200,50,50)
    ...
    local function makeStat()
        ...
        box.Size=UDim2.new(0,56,1,0); box.BackgroundColor3=Color3.fromRGB(40,0,60)
    ...
    local timeLbl  = makeStat()   -- FIX: timeLbl sekarang dipakai
    ...
    -- FIX: fmtTime sekarang dipakai → tidak ada dead code
    ...
    local sessionStart=tick()   -- FIX: sessionStart dipakai di timeLbl
            ...
            if Config.CustomWebhook and Config.CustomWebhookUrl~="" then
                pcall(function()
                    ...
                    local payload=HttpService:JSONEncode({
                        username="MNA HUB",
                        embeds={{
                            title="Caught Fish!", color=0x00aaff,
                            fields={
                            ...
                            footer={text="MNA HUB V11.4"}
                        }}
                    })
                    if typeof(request)=="function" then
                        request({ Url=Config.CustomWebhookUrl, Method="POST",
...
ToggleBtn.Size=UDim2.new(0,48,0,48); ToggleBtn.Position=UDim2.new(0.05,0,0.04,0)
ToggleBtn.BackgroundColor3=Color3.fromRGB(110,15,15)
ToggleBtn.Image="rbxassetid://7733715400"; ToggleBtn.Draggable=true; ToggleBtn.BorderSizePixel=0
Instance.new("UICorner",ToggleBtn).CornerRadius=UDim.new(1,0)
local btnStroke=Instance.new("UIStroke",ToggleBtn)
btnStroke.Thickness=2; btnStroke.Color=Color3.fromRGB(200,50,50)
...
WindUI:Notify({ Title="MNA HUB", Content="Loaded! Remotes: "..loadedCount,
    Duration=4, Icon="fish" })
...
InfoTab:Paragraph({ Title="Remote Status",
    Content="Loaded: "..loadedCount.." | Failed: "..failedCount })
InfoTab:Button({ Title="Fix Remote", Desc="Reload semua remote",
    Callback=function()
        local l,f=loadRemotes(); UB_init()
        NotifySuccess("Remotes","Loaded: "..l.." | Failed: "..f)
    end
})
InfoTab:Keybind({ Title="Toggle UI", Desc="Buka / tutup jendela",
    Value="RightShift",
    Callback=function(v)
...
PlayersTab:Section({ Title="Movement", Icon="move" })

PlayersTab:Slider({ Title="Walk Speed",
    Value={ Min=16, Max=200, Default=16 }, Step=1,
    Callback=function(val)
        local char=LocalPlayer.Character
        if char then local h=char:FindFirstChildOfClass("Humanoid"); if h then h.WalkSpeed=val end end
    end
})
PlayersTab:Slider({ Title="Jump Power",
    Value={ Min=50, Max=500, Default=50 }, Step=10,
    Callback=function(val)
        local char=LocalPlayer.Character
        if char then local h=char:FindFirstChildOfClass("Humanoid")
            if h then h.UseJumpPower=true; h.JumpPower=val end end
    end
})
PlayersTab:Button({ Title="Reset Speed & Jump",
    Callback=function()
        local char=LocalPlayer.Character
        if char then local h=char:FindFirstChildOfClass("Humanoid")
            if h then h.WalkSpeed=16; h.UseJumpPower=true; h.JumpPower=50 end end
...
-- FIX typo: "INFINITE JUMPI" → "Infinite Jump"
PlayersTab:Toggle({ Title="Infinite Jump", Value=false,
...
-- FIX typo: "NOCLIP" cukup
PlayersTab:Toggle({ Title="Noclip", Value=false,
    Callback=function(v)
        _G.Noclip=v
        if v then
            task.spawn(function()
                while _G.Noclip do task.wait(0.1)
                    local char=LocalPlayer.Character
                    if char then
...
-- FIX typo: "FREZE CHARACTER" → "Freeze Character"
local freezeConn,frozenCF
PlayersTab:Toggle({ Title="Freeze Character", Value=false,
    Callback=function(v)
...
-- FIX typo: "WALK OL WATER" → "Walk on Water"
PlayersTab:Toggle({ Title="Walk on Water", Desc="Berjalan di atas permukaan air", Value=false,
    Callback=function(v) setWalkOnWater(v) end
})
PlayersTab:Toggle({ Title="Hide NameTag", Desc="Sembunyikan nama di atas kepala", Value=false,
...
FishTab:Section({ Title="Ultra Blatant 3N", Icon="zap" })

local rodSettings = {
    ["1.  2-Notif  (2.998s)"]         = { CompleteDelay=2.998  },
    ["2.  Ultra 3N (2.799s)"]          = { CompleteDelay=2.7988 },
    ["3.  Diamond / Element (2.890s)"] = { CompleteDelay=2.890  },
    ["4.  GF / Bambu (3.7s)"]          = { CompleteDelay=3.7    },
    ["5.  4-Notif Visual (4.8s)"]      = { CompleteDelay=4.8    },
}
local rodNames={}
for n in pairs(rodSettings) do table.insert(rodNames,n) end
table.sort(rodNames)

FishTab:Dropdown({ Title="Template Rod", Values=rodNames, Value=rodNames[1],
    Callback=function(v)
        local s=rodSettings[v]; if s then Config.UB.Settings.CompleteDelay=s.CompleteDelay end
    end
})
FishTab:Input({ Title="Complete Delay (detik)", Placeholder=tostring(Config.UB.Settings.CompleteDelay),
    Callback=function(t)
        local n=tonumber(t)
...
FishTab:Slider({ Title="UB Notif Duration (× delay)",
    Desc="Makin tinggi = notif makin lama di layar",
    Value={ Min=10, Max=50, Default=20 }, Step=1,
    Callback=function(v)
        Config.UBNotifDurationMult=v/10
        NotifyInfo("UB Notif","Durasi: "..Config.UBNotifDurationMult.."× delay")
    end
})
FishTab:Toggle({ Title="Ultra Blatant 3N", Value=false,
    Callback=function(v) needCast=true; onToggleUB(v) end
})
FishTab:Toggle({ Title="amBlantat  (visual + N notif/catch)", Desc="Aktifkan notif visual real", Value=false,
    Callback=function(v)
        Config.amblatant=v; saveCount=0
        HookRemote("RE/FishCaught","FishCaught")
        HookRemote("RE/CaughtFishVisual","CaughtVisual")
        HookRemote("RE/ObtainedNewFishNotification","FishNotif")
        needCast=true
        if v then onToggleUB(true)
        else Config.amblatant=false; NotifyWarning("amBlantat","Dimatikan. UB tetap jalan.") end
    end
})
FishTab:Toggle({ Title="Random Cast  (Anti-Detect)", Value=false,
    Callback=function(v) Config.antiOKOK=v end
})

FishTab:Divider()
FishTab:Section({ Title="Instant Bobber", Icon="crosshair" })
FishTab:Toggle({ Title="Instant Bobber", Desc="Bobber langsung muncul di posisi cast", Value=false,
    Callback=function(v)
        Config.InstantBobber=v; patchInstantBobber(v)
        if v then NotifySuccess("Instant Bobber","Aktif!") else NotifyWarning("Instant Bobber","Dimatikan.") end
    end
})

FishTab:Divider()
FishTab:Section({ Title="Custom Skin Animation", Icon="sparkles" })
FishTab:Paragraph({ Title="Info", Content="Pilih skin rod, lalu aktifkan toggle." })

-- FIX: default value "Eclipse" — cocok dengan SkinDatabase
local ubSkinNames={
...
FishTab:Dropdown({ Title="Skin Rod Animation", Values=ubSkinNames, Value="Eclipse",
    Callback=function(v) Config.SelectedSkinId=v; SkinAnimation.Switch(v) end
})
FishTab:Toggle({ Title="Enable Skin Animation", Desc="Aktifkan animasi catch custom", Value=false,
    Callback=function(v)
        Config.SkinAnimEnabled=v
        if v then
            SkinAnimation.Switch(Config.SelectedSkinId); SkinAnimation.Enable()
            NotifySuccess("Skin Anim",Config.SelectedSkinId.." aktif!")
        else
            SkinAnimation.Disable(); NotifyWarning("Skin Anim","Dimatikan.")
        end
    end
})
...
FishTab:Slider({ Title="Jumlah Notif per Catch", Desc="Default: 3",
    Value={ Min=1, Max=10, Default=3 }, Step=1,
    Callback=function(v) Config.NotifCount=v; NotifyInfo("amBlantat","Jumlah: "..v.."× per catch") end
})
FishTab:Slider({ Title="Delay Notif  (× 0.01 detik)", Desc="Default: 10 = 0.1 s",
    Value={ Min=0, Max=100, Default=10 }, Step=1,
    Callback=function(v) Config.NotifDelay=v/100; NotifyInfo("amBlantat Delay","Delay: "..Config.NotifDelay.."s") end
})

FishTab:Divider()
FishTab:Section({ Title="Legit Fishing", Icon="shield-check" })
FishTab:Toggle({ Title="Legit Auto Catch", Value=false,
    Callback=function(v)
        Config.AutoCatch=v
...
FishTab:Input({ Title="Catch Delay (detik)", Placeholder="0.7",
    Callback=function(t)
        local n=tonumber(t); if n and n>=0 then Config.CatchDelay=n; NotifySuccess("Catch Delay","Set: "..n.."s") end
    end
})
FishTab:Toggle({ Title="Perfection Enchant", Value=false,
    Callback=function(v)
        Config.autoFishing=v; CallRemote(Events.UpdateAutoFishing,v)
        if v then NotifySuccess("Perfection","Aktif!") else NotifyWarning("Perfection","Dimatikan.") end
    end
})

FishTab:Divider()
FishTab:Section({ Title="Auto Sell", Icon="shopping-cart" })
FishTab:Dropdown({ Title="Metode Sell", Values={"Delay","Count"}, Value="Delay",
    Callback=function(v) Config.AutoSellMethod=v; if Config.AutoSellState then RunAutoSellLoop() end end
})
FishTab:Input({ Title="Sell Value", Desc="Delay = detik  |  Count = jumlah catch", Placeholder="50",
    Callback=function(t)
        local n=tonumber(t); if n and n>0 then Config.AutoSellValue=n; NotifySuccess("Sell Value","Set: "..n) end
    end
})
FishTab:Toggle({ Title="Enable Auto Sell", Value=false,
    Callback=function(v)
...
FishTab:Button({ Title="Sell All Now",
    Callback=function()
        if Events.sell then
            pcall(function() Events.sell:InvokeServer({}) end)
...
FishTab:Dropdown({ Title="Filter Rarity",
    Values={"Common","Uncommon","Rare","Epic","Legendary","Mythic","SECRET"}, Multi=true,
    Callback=function(v) Config.SelectedRarities=type(v)=="table" and v or (v~="" and {v} or {}) end
})
FishTab:Dropdown({ Title="Filter Mutation",
    Values={"Galaxy","Corrupt","Gemstone","Fairy Dust","Midnight","Color Burn",
    ...
    Multi=true,
    Callback=function(v) Config.SelectedMutations=type(v)=="table" and v or (v~="" and {v} or {}) end
})
FishTab:Toggle({ Title="Auto Favorite", Value=false,
    Callback=function(v)
...
FishTab:Toggle({ Title="Auto Unfavorite", Value=false,
    Callback=function(v)
...
-- ============================================================
--  TAB: EVENT  (gabungan Easter + Auto Event)
-- ============================================================
local EventTab = Window:Tab({ Title="Event", Icon="gift" })
EventTab:Section({ Title="Easter Hunt", Icon="search" })
EventTab:Paragraph({ Title="Info", Content="Auto collect telur Easter di map. TP dulu ke Easter Island." })
EventTab:Button({ Title="TP → Easter Island",
    Callback=function() teleportTo("Easter Island"); NotifySuccess("TP","Easter Island!") end
})
EventTab:Button({ Title="TP → Easter Cove",
    Callback=function() teleportTo("Easter Cove"); NotifySuccess("TP","Easter Cove!") end
})
EventTab:Toggle({ Title="Auto Egg Hunt", Desc="Auto kumpulkan telur Easter", Value=false,
    Callback=function(v)
...
EventTab:Divider()
EventTab:Section({ Title="Buy Egg", Icon="shopping-bag" })
local eggNames={}
for n in pairs(EggShopList) do table.insert(eggNames,n) end
table.sort(eggNames)
EventTab:Dropdown({ Title="Pilih Egg", Values=eggNames, Value=eggNames[1],
    Callback=function(v)
        local data=EggShopList[v]
        if data then Config.SelectedEggId=data.id; NotifyInfo("Egg","Dipilih: "..v) end
    end
})
EventTab:Toggle({ Title="Auto Buy Egg", Desc="Auto beli egg yang dipilih", Value=false,
    Callback=function(v)
        Config.AutoBuyEgg=v
        if v then
...
EventTab:Button({ Title="Buy Once",
    Callback=function()
        ...
        if Events.BuyEgg then
            ...
            NotifySuccess("Buy Egg","Dibeli!")
        else NotifyError("Buy Egg","Remote tidak ditemukan!") end
    end
})

EventTab:Divider()
EventTab:Section({ Title="Auto Event Interact", Icon="calendar" })
EventTab:Toggle({ Title="Auto Event", Desc="Auto interact semua event object di map", Value=false,
    Callback=function(v)
        Config.AutoEvent=v
...
-- ============================================================
--  TAB: MAIN  (Enchant, Cave, Crystal, Totem)
-- ============================================================
local MainTab = Window:Tab({ Title="Main", Icon="settings-2" })
MainTab:Section({ Title="Enchant", Icon="sparkles" })

local enchantStatusPara = MainTab:Paragraph({ Title="Enchant Status",
...
task.spawn(function()
    ...
    while true do task.wait(2)
        pcall(function()
            ...
            if rod~=lastR or en~=lastE or stones~=lastS then
                enchantStatusPara:SetDesc(string.format(
                    ...
                    rod,en,stones))
                lastR,lastE,lastS=rod,en,stones
            end
        end)
    end
end)

MainTab:Dropdown({ Title="Stone Type",
    Values={"Enchant Stones","Evolved Enchant Stone"}, Value="Enchant Stones",
    Callback=function(v) _G.SelectedStoneType=v end
})
MainTab:Dropdown({ Title="Target Enchant",
...
MainTab:Toggle({ Title="Auto Enchant", Value=false,
    Callback=function(v)
        _G.AutoEnchant=v
        if v then
            task.spawn(function()
                while _G.AutoEnchant do
                    pcall(function()
                        ...
                        local tid=STONE_IDS[_G.SelectedStoneType or "Enchant Stones"]
                        local stoneUUID=nil
                        for _,item in ipairs(inv.Items) do if item.Id==tid then stoneUUID=item.UUID; break end end
...
MainTab:Button({ Title="Teleport → Altar",
    Callback=function()
        local hrp=getHRP()
        if hrp then hrp.CFrame=CFrame.new(3234.837,-1302.855,1398.391); NotifySuccess("TP","Berhasil ke Altar!") end
    end
})
MainTab:Button({ Title="Double Enchant  (sekali)", Desc="Equip stone → hotbar → activate altar",
    Callback=function()
        task.spawn(function()
            if not Events.activateAltar then NotifyError("Double Enchant","Remote altar tidak ditemukan!"); return end
...
MainTab:Button({ Title="Fix Rod", Desc="Gunakan jika rod / skin tidak bisa diganti",
    Callback=function()
        local done=false
        if Events.CancelFishing then pcall(function() CallRemote(Events.CancelFishing) end); done=true
        elseif Config.UB.Remotes.CancelFishingInputs then pcall(function() CallRemote(Config.UB.Remotes.CancelFishingInputs) end); done=true end
...
MainTab:Button({ Title="Open Cave Wall", Desc="TNT × 4 → gainAccessToMaze",
    Callback=function()
        task.spawn(function()
...
MainTab:Toggle({ Title="Auto Open Pirate Chest", Value=false,
    Callback=function(v)
...
MainTab:Button({ Title="Consume Crystal Now",
    Callback=function()
        if Events.ConsumeCaveCrystal then
            pcall(function() Events.ConsumeCaveCrystal:InvokeServer() end)
            task.wait(1.5); equipRod(); NotifySuccess("Crystal","Dikonsumsi!")
        else NotifyError("Crystal","Remote tidak ditemukan!") end
    end
})
MainTab:Toggle({ Title="Auto Consume Crystal  (30 menit)", Value=false,
    Callback=function(v)
        _G.AutoCrystal=v
        if v then
...
MainTab:Dropdown({ Title="Pilih Totem",
    Values={"Pilih Totem","Luck Totem","Mutation Totem","Shiny Totem","Love Totem"},
    Value="Pilih Totem",
    Callback=function(v) Config.SelectedTotemID=totemData[v] or 0; NotifyInfo("Totem","Dipilih: "..v) end
})
MainTab:Toggle({ Title="Auto Spawn Totem  (1 jam)", Value=false,
    Callback=function(v)
        Config.AutoTotem=v
        if v then
...
CosmeticTab:Section({ Title="Aura", Icon="star" })
CosmeticTab:Paragraph({ Title="Info", Content="Efek partikel di sekitar karakter (client-side)." })
...
CosmeticTab:Dropdown({ Title="Pilih Aura", Values=auraNames, Value="None",
    Callback=function(v) selectedAura=v end
})
CosmeticTab:Button({ Title="Apply Aura",
    Callback=function() applyAura(selectedAura) end
})
CosmeticTab:Button({ Title="Remove Aura",
...
TpTab:Dropdown({ Title="Pilih Lokasi", Values=locNames, Value=locNames[1],
    Callback=function(v) selectedLoc=v end
})
TpTab:Button({ Title="Teleport!",
    Callback=function() teleportTo(selectedLoc); NotifySuccess("TP","→ "..selectedLoc) end
})
...
TpTab:Dropdown({ Title="Pilih Player", Values=getPlayerList(),
    Callback=function(v) selectedPlayer=v end
})
TpTab:Button({ Title="Teleport ke Player",
    Callback=function()
        ...
        if target and target.Character then
            ...
            local hrp=getHRP()
            if hrp and tH then
                hrp.CFrame=CFrame.new(tH.Position+Vector3.new(0,3,0))
                NotifySuccess("TP","→ "..selectedPlayer)
            end
...
TpTab:Divider()
TpTab:Section({ Title="Event TP  (Platform)", Icon="crosshair" })
TpTab:Toggle({ Title="Auto Leviathan Hunt TP", Value=false,
    Callback=function(v)
        _G.AutoLev=v
        if v then
            local hasTP=false
...
TpTab:Paragraph({ Title="Info", Content="Pilih event → aktifkan. Script scan & TP ke platform melayang." })

local eventTPNames={}
for n in pairs(eventTPData) do table.insert(eventTPNames,n) end
table.sort(eventTPNames)

TpTab:Dropdown({ Title="Pilih Event", Values=eventTPNames, Multi=true,
    Callback=function(v)
        selectedAutoEvents=type(v)=="table" and v or (v~="" and {v} or {})
    end
})
TpTab:Toggle({ Title="Auto Event Teleport", Desc="TP ke platform melayang di atas event aktif", Value=false,
    Callback=function(v)
        autoEventTPEnabled=v
        if v then
            if #selectedAutoEvents==0 then NotifyError("Event TP","Pilih minimal 1 event!"); autoEventTPEnabled=false; return end
            if autoEventThread then pcall(function() task.cancel(autoEventThread) end) end
            autoEventThread=task.spawn(runAutoEventTP)
            NotifySuccess("Event TP","Aktif!")
        else
            destroyEventPlatform()
...
ShopTab:Dropdown({ Title="Pilih Weather", Values=wxNames, Multi=true,
    Callback=function(v) selectedWeathers=type(v)=="table" and v or (v~="" and {v} or {}) end
})
ShopTab:Button({ Title="Buy Selected Weather",
    Callback=function()
...
ShopTab:Toggle({ Title="Auto Buy Weather", Value=false,
    Callback=function(v)
        _G.AutoWeather=v
        if v then
            if #selectedWeathers==0 then NotifyError("Error","Pilih weather dulu!"); _G.AutoWeather=false; return end
            task.spawn(function()
...
MiscTab:Section({ Title="Performance", Icon="zap" })

local stopAnimConns={}
MiscTab:Toggle({ Title="No Animation", Value=false,
    Callback=function(v)
        for _,c in ipairs(stopAnimConns) do c:Disconnect() end; stopAnimConns={}
        if v then
            local char=LocalPlayer.Character
            if char then
                local hum=char:FindFirstChildOfClass("Humanoid")
            ...
            NotifyInfo("Anim","Animasi dimatikan.")
        else NotifyInfo("Anim","Animasi normal.") end
    end
})
MiscTab:Toggle({ Title="FPS Booster", Value=false,
    Callback=function(v)
        if v then
            for _,obj in pairs(workspace:GetDescendants()) do
                pcall(function()
            ...
            NotifyInfo("FPS Booster","Normal.")
        end
    end
})
MiscTab:Toggle({ Title="Disable Obtained Notification", Desc="Sembunyikan notif item obtained", Value=false,
...
local function DisableCtrl(ctrl)
    ...
    local d={functions={}}
    for k,v in pairs(ctrl) do if type(v)=="function" then d.functions[k]=v; ctrl[k]=function()end end end
    _backup[ctrl]=d
end
local function EnableCtrl(ctrl)
    local d=_backup[ctrl]; if not d then return end
    for k,v in pairs(d.functions) do ctrl[k]=v end; _backup[ctrl]=nil
end

MiscTab:Toggle({ Title="Disable VFX", Value=false,
    Callback=function(v)
        if Controllers.VFX then if v then DisableCtrl(Controllers.VFX) else EnableCtrl(Controllers.VFX) end end
    end
})
MiscTab:Toggle({ Title="Disable Cutscene", Value=false,
    Callback=function(v)
        if Controllers.Cutscene then if v then DisableCtrl(Controllers.Cutscene) else EnableCtrl(Controllers.Cutscene) end end
    end
})

MiscTab:Divider()
MiscTab:Section({ Title="Stats Panel", Icon="bar-chart" })
MiscTab:Toggle({ Title="Ping Panel  (pojok kanan atas)", Desc="Ping + FPS realtime", Value=false,
    Callback=function(v)
        if v then createPingPanel(); NotifySuccess("Ping Panel","Aktif!")
        else destroyPingPanel(); NotifyWarning("Ping Panel","Disembunyikan.") end
    end
})
MiscTab:Toggle({ Title="Realtime Stats Panel", Desc="Draggable · Ping · FPS · Notif · Timer", Value=false,
    Callback=function(v)
...
MiscTab:Button({ Title="Cek FPS & Ping  (sekali)",
    Callback=function()
...
MiscTab:Toggle({ Title="Enable Webhook", Value=false,
    Callback=function(v) Config.CustomWebhook=v end
})
MiscTab:Input({ Title="Webhook URL", Placeholder="https://discord.com/api/webhooks/...",
    Callback=function(t)
...
-- ============================================================
task.wait(0.5)
WindUI:Notify({ Title="MNA HUB  V11.4", Content="✅ Semua fitur loaded! amBlantat: "..Config.NotifCount.."× per catch",
